...
2. **Build and Install Driver:** Build the kernel project to generate the required `.ko` driver file. Following this, if the driver was build correctly it will be installed using ```insmod nxp_simtemp.ko```; otherwise the process will be stoped.
...
```nxp_simtemp_cli``` support the following commands:
